...
# [0.7340069825404124, 0.6328887992869787, 0.0006737336701407859, 1.1102230246251565e-16]

class RestU(BackpropBase):
    ...
    def update_matrices(self):
        ...
        for i, layer_i in enumerate(self.model.layers[1:], 1):
            assert isinstance(layer_i, LIF)
            diff_J = diff_Js[i]
            diff_uu = layer_i.linear.weight.unsqueeze(0) * self.model.layers[i - 1].spatial_derivative(y='s',
                                                                                                       x='u').unsqueeze(
                1)
            ...
            for j, layer_j in enumerate(self.model.layers[:i]):
                ...
                if i == self.num_layers - 1:
                    self.part1s.append(part1)

        del diff_Js

    @torch.no_grad()
    def assign_grad(self):
        grad_last = self.model.layers[-1].mem.grad
        for i, layer in enumerate(self.model.layers):
            assert isinstance(layer, LIF)
            if i == self.num_layers - 1:
                if layer.recurrent is None:
                ...
                if layer.recurrent is None:
                    grads = torch.einsum('bi,bij->ij', layer.mem.grad, self.M[i]) + \
                            ...
                            # torch.einsum('bc,bcik,bjk->ij', grad_last, self.part1s[i], self.Bs[i]) / self.dim
                else:
                    tmp = torch.einsum('bc,bcik,bjk->ij', grad_last, self.part1s[i], self.U[i].B) / self.dim
                    # tmp = torch.einsum('bc,bcik,bjk->ij', grad_last, self.part1s[i], self.Bs[i]) / self.dim
                    grads = (torch.einsum('bi,bij->ij', layer.mem.grad, self.M[i]) + tmp)

            # print(torch.norm(grads).item())
...
def get_restu(model, create_bptt=False, temporal_detach_bptt=False, num=1, init="randn") -> RestU:
    assert model.neuron_type == 'lif'
    return RestU(model, create_bptt, temporal_detach_bptt, num, init)
