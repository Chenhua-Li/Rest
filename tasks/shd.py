...
class AlignedSHD(BinnedSHD):
    def __init__(
        ...
        max_frame = max(super(AlignedSHD, self).__getitem__(i)[0].shape[0] for i in range(self.length))
...
class SHDTask(Task):
    def __init__(self, root='./data/SHD', n_bins=1, time_step=25):
        self.root = root
        # self.train_dataset = SHDTaskDataset(root, train=True)
        # self.test_dataset = SHDTaskDataset(root, train=False)
        self.train_dataset = AlignedSHD(root, n_bins, train=True, data_type='frame',
                                duration=time_step )
        self.test_dataset = AlignedSHD(root, n_bins, train=False, data_type='frame',
                                duration=time_step )
